...
<!-- Business Type Dropdown -->
        ...
        <div class="col-sm-4">
    <div class="form-group">
        {!! Form::label('business_type', __('Business Type') . ':') !!}
            {!! Form::select('business_type',     collect($business_type)->pluck('label', 'value')->toArray(),  !empty($duplicate_product->business_type) ? $duplicate_product->business_type : null,   ['placeholder' => __('messages.please_select'), 'class' => 'form-control select2']
            ); !!}
...
<div class="col-sm-4">
    <div class="form-group">
        {!! Form::label('scenario_code', __('Scenario') . ':') !!}
        {!! Form::select('scenario_code', 
            [], 
            !empty($duplicate_product->scenario_code) ? $duplicate_product->scenario_code : null,
            ['placeholder' => __('messages.please_select'), 'class' => 'form-control select2', 'id' => 'scenario_code']
        ); !!}
    </div>
</div>
...
$(document).ready(function() {
    ...
    $('#business_type').change(function() {
        ...
        var $scenarioDropdown = $('#scenario_code');
        
        if(businessType) {
            $scenarioDropdown.empty().append('<option value="">Loading scenarios...</option>');
            
            // Get scenarios for selected business type
            $.ajax({
                url: '/fbr-scenarios/by-business-type',
                type: 'GET',
                data: { business_type: businessType },
                success: function(response) {
                if (response.success && response.data.scenarios.length > 0) {
                    // Clear and enable dropdown
                    $scenarioDropdown.empty().append('<option value="">Select Scenario</option>');
                    
                    // Add scenarios to dropdown
                    $.each(response.data.scenarios, function(index, scenario) {
                        $scenarioDropdown.append(
                            $('<option></option>')
                                .val(scenario.value) // This will be the raw code (e.g., "SN001")
                                .text(scenario.label) // This will show "SN001 - Scenario Name"
                                .data('description', scenario.description)
                        );
                    });
                    
                    $scenarioDropdown.prop('disabled', false);
                    
                    // If editing, select the saved scenario
                    @if(isset($product) && $product->scenario_code)
                        $scenarioDropdown.val('{{ $product->scenario_code }}');
                    @endif
                } else {
                    $scenarioDropdown.append('<option value="">No scenarios available</option>');
                }
            },

                error: function() {
                    $scenarioDropdown.empty().append('<option value="">Error loading scenarios</option>');
                }
            });
        } else {
            $scenarioDropdown.empty().append('<option value="">Select business type first</option>').prop('disabled', true);
        }
    });

    // Optional: Show scenario details when selected
    $('#scenario_code').change(function() {
        if($(this).val()) {
            // You could fetch and display scenario details here
            // using your /{scenarioCode} endpoint
        }
    });
});
</script>
