...
class FbrScenariosSeeder extends Seeder
{
    /**
    ...
    {
        // Business type scenarios mapping
        $businessScenarios = [
        ...
        // Complete scenarios data - All 28 scenarios
        $scenarios = [
            'SN001' => [
        ...
        foreach ($scenarios as $code => $scenario) {
            ...
            $businessTypesForScenario = [];
            foreach ($businessScenarios as $businessType => $scenarioCodes) {
                if (in_array($code, $scenarioCodes)) {
                    $businessTypesForScenario[] = $businessType;
                }
            }

            $scenarioId = DB::table('fbr_scenarios')->insertGetId([
                'scenario_code' => $code,
                'name' => $scenario['name'],
                'description' => $scenario['description'],
                'business_types' => json_encode($businessTypesForScenario),
                'seller_ntn_cnic' => $scenario['sample_data']['sellerNTNCNIC'] ?? null,
                'seller_business_name' => $scenario['sample_data']['sellerBusinessName'] ?? null,
                'buyer_ntn_cnic' => $scenario['sample_data']['buyerNTNCNIC'] ?? null,
                'buyer_business_name' => $scenario['sample_data']['buyerBusinessName'] ?? null,
                'buyer_registration_type' => $scenario['sample_data']['buyerRegistrationType'] ?? null,
                'invoice_ref_no' => $scenario['sample_data']['invoiceRefNo'] ?? null,
                'created_at' => now(),
                'updated_at' => now(),
            ]);

            // Insert scenario item
            if (isset($scenario['sample_data']['item'])) {
                $item = $scenario['sample_data']['item'];
                DB::table('fbr_scenario_items')->insert([
                    ...
                    'value_sales_excluding_st' => $item['valueSalesExcludingST'] ?? 0,
                    'sales_tax_applicable' => $item['salesTaxApplicable'] ?? 0,
                    'total_values' => $item['totalValues'] ?? 0,
                    'fixed_notified_value_or_retail_price' => $item['fixedNotifiedValueOrRetailPrice'] ?? 0,
                    'sales_tax_withheld_at_source' => $item['salesTaxWithheldAtSource'] ?? 0,
        ...
        // Insert business type mappings for all existing scenarios
        foreach ($businessScenarios as $businessType => $scenarioCodes) {
            foreach ($scenarioCodes as $scenarioCode) {
                // Only insert mapping if scenario exists in our scenarios array
                if (array_key_exists($scenarioCode, $scenarios)) {
                    DB::table('fbr_business_type_scenarios')->insert([
